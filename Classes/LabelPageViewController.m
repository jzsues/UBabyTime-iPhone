...
@implementation LabelPageViewController
...
- (void)unloadSubviews {
    for(int i = 0; i < _labelViews.count; i++) {
        LabelViewController *label = ((LabelViewController *)[_labelViews objectAtIndex:i]);
...
- (void)removeLabelPostAnimation:(NSUInteger)index {
    for(int i = index; i < 4; i++) {
        LabelViewController *label = [_labelViews objectAtIndex:i];
        CGRect oldFrame = label.view.frame;
        CGRect newFrame;
...
#pragma mark -
#pragma mark LabelViewController delegate
...
- (void)labelView:(LabelViewController *)labelView didSelectLabelAtIndex:(NSUInteger)index {
    ...
    LabelViewController *label = ((LabelViewController *)[_labelViews objectAtIndex:index]);
    [self.view addSubview:label.view];
...
- (void)activateLastLabel:(LabelInfo *)info delayed:(BOOL)delay{
    ...
    LabelViewController *label = [_labelViews objectAtIndex:labelIndex];
...
- (void)forceRefreshParentLabelPageData {
    ...
    for(LabelViewController *label in _labelViews) {
